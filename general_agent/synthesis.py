...
class TaskBundle:
    ...
    def verify(
        self, tools: Dict[str, Any], answer: Any, force_local: bool = False
    ) -> bool:
    ...
    def _run_in_sandbox_fusion(
        self, code: str, tools: Dict[str, Any], func_name: str, *args: Any
    ) -> Any:
...
"""
        ...
        if result.get("return_code", 0) != 0 or result.get("status") == "error":
            raise RuntimeError(
                f"SandboxFusion execution failed: {result.get('stderr', 'Unknown error')}"
            )
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to parse SandboxFusion output: {output[:200]}. Error: {e}"
                )
...
class EnvironmentSynthesizer:
    ...
    def _extract_tool_calls(solution_code: str) -> set[str]:
        ...
        tool_calls = re.findall(
            r"tools\[['\"](\w+)['\"]\]|tools\.(\w+)\s*\(", solution_code or ""
        )
    ...
    def build_context(
        self, category: str, sandbox: Path, use_sandbox_fusion: bool = True
    ) -> SynthesisContext:
        ...
        return SynthesisContext(
            category=category, sandbox=sandbox, db=db, registry=registry, llm=self.llm
        )
    ...
    def synthesize_tools(
        self, ctx: SynthesisContext, additional_context: str = ""
    ) -> None:
        ...
        for spec in tools:
            ...
            def make_handler(key: str):
                def base_handler(*args: Any, **kwargs: Any) -> Any:
                    ...
                    logger.debug(
                        "Tool '%s' called with args=%s, kwargs=%s", key, args, kwargs
                    )
                    candidate: Any = None
                    if args:
                        candidate = args[0]
                    ...
                    if isinstance(result, list) and result:
                        ...
                        if any(
                            word in key.lower()
                            for word in ["matcher", "finder", "neighborhood"]
                        ):
                            # For matchers/finders: return list of names/titles
                            result = [r.get("title", str(r)) for r in result[:5]]
                        elif any(
                            word in key.lower()
                            for word in ["recommendation", "seasonal", "advisor"]
                        ):
                            # For recommendations: return summary text from first few matches
                            result = [
                                r.get("summary", r.get("title", str(r)))
                                for r in result[:3]
                            ]
                        elif any(
                            word in key.lower()
                            for word in ["categorizer", "attraction", "activity"]
                        ):
                            ...
                            word in key.lower()
                            for word in ["checker", "analyzer", "validator"]
                        ):
    ...
    def augment_toolset(
        self, ctx: SynthesisContext, bundle: TaskBundle, failure_reason: str
    ) -> bool:
        ...
        needs_augmentation = (
            ...
            or (
                len(failure_reason) > 50
                and "verification returned False" in failure_reason
            )
        )
        ...
        for spec in new_tools:
            ...
            def make_handler(key: str):
                def base_handler(*args: Any, **kwargs: Any) -> Any:
                    ...
                    if any(
                        word in key.lower()
                        for word in ["checker", "analyzer", "validator", "matcher"]
                    ):
                        if isinstance(result, list) and result:
                    ...
                    logger.debug(
                        "Augmented tool '%s' returned %s", key, type(result).__name__
                    )
                    return result
    ...
    def smart_db_query(
        self, records: List[Dict[str, Any]], tool_key: str, query: str
    ) -> List[Dict[str, Any]]:
    ...
    def refine_task(self, ctx: SynthesisContext, prev: TaskBundle) -> TaskBundle:
        ...
        unused_tools = [
            t for t in all_tools if t not in prev_tools and t not in ["bash", "search"]
        ]
        ...
        for attempt in range(3):
            raw = ctx.llm.simple_complete(
                prompt, temperature=0.9 + attempt * 0.05, max_tokens=1200
            )
            try:
                ...
                name_changed = (
                    bool(new_name)
                    and new_name.strip().lower() != prev.name.strip().lower()
                )
                code_changed = (
                    bool(new_code) and new_code.strip() != prev.solution_code.strip()
                )
                desc_changed = (
                    bool(new_desc) and new_desc.strip() != prev.description.strip()
                )
                ...
                if name_changed or code_changed or desc_changed or tools_changed:
                    return TaskBundle(
                        ...
                        verification_code=data.get(
                            "verification_code", prev.verification_code
                        ),
                    )
            except json.JSONDecodeError:
                continue
        ...
        logger.info(
            "LLM did not provide a sufficiently different task; using fallback variant"
        )
        return self._create_fallback_refined_task(ctx, prev)

    def _create_fallback_refined_task(
        self, ctx: SynthesisContext, prev: TaskBundle
    ) -> TaskBundle:
        """Create a fallback refined task when LLM fails to generate a different one."""
        all_tools = [
            t["name"]
            for t in ctx.registry.describe()
            if t["name"] not in ["bash", "search"]
        ]
...
{chr(10).join(tool_calls)}
    ...
    def repair_bundle(
        ...
        try:
            ...
            logger.warning(
                "LLM repair did not return JSON; keeping original task: %s", raw
            )
    ...
    def ensure_valid(
        ...
        bundle = self._ensure_substantive_task(
            ctx, bundle, "Initial validation quality gate"
        )
        ...
        def fallback(*args: Any, **kwargs: Any) -> List[Dict[str, Any]]:
            ...
            if candidate is None:
                ...
                result = ctx.db.query("title", text) or [
                    ...
                    if text in r.get("title", "") or text in r.get("summary", "")
                ]
        ...
        for attempt in range(self.max_validation_rounds + 1):
            ...
            try:
                ...
                last_error = str(exc)
                logger.warning(
                    "Task %s raised during execution: %s", bundle.name, last_error
                )
                valid = False
            else:
                if not valid:
            ...
            bundle = self._ensure_substantive_task(
                ctx, bundle, "Post-repair quality gate"
            )
        ...
        if fail_soft:
            ...
            return bundle, None
        raise RuntimeError(
            f"Task failed validation repeatedly: {bundle.name}; last error: {last_error}"
        )
    ...
    def _ensure_substantive_task(
        ...
        for _ in range(3):
            ...
            bundle = self.repair_bundle(
                ctx, bundle, f"{base_reason}; tool_calls={list(tool_calls)}"
            )
        return bundle
    ...
    def synthesize(
        ...
        ctx = self.build_context(
            category, sandbox, use_sandbox_fusion=use_sandbox_fusion
        )
        ...
        for step in range(1, rounds):
            ...
            if validate:
                ...
                if missing_tools:
                    ...
                    self.augment_toolset(
                        ctx, current, f"Task requires tools: {missing_tools}"
                    )

                print(f"   ⏳ 验证任务...")
                try:
                    current, answer = self.ensure_valid(
                        ctx, current, fail_soft=fail_soft
                    )
                    if answer is not None:
                        print(f"   ✅ 验证通过!")
                    else:

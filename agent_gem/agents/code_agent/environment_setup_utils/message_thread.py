"""
MessageThread and FunctionCallIntent implementation.

This module provides message thread management and function call intent handling,
adapted from app.data_structures for use in code_agent module.
"""

from __future__ import annotations

import json
import base64
from collections.abc import Mapping
from dataclasses import dataclass
from pprint import pformat
from typing import Any

try:
    from openai.types.chat import ChatCompletionMessageToolCall
    from openai.types.chat.chat_completion_message_tool_call import (
        Function as OpenaiFunction,
    )
except ImportError:
    # Fallback if openai is not available
    ChatCompletionMessageToolCall = Any
    OpenaiFunction = Any


@dataclass
class MethodId:
    """Method identifier for tracking method calls."""
    
    class_name: str
    method_name: str

    def __str__(self):
        if self.class_name:
            return f"{self.class_name}.{self.method_name}"
        return self.method_name

    def __hash__(self):
        return hash((self.class_name, self.method_name))


class FunctionCallIntent:
    """An intent to call a tool function.
    
    This object is created from OpenAI API response.
    """

    def __init__(
        self,
        func_name: str,
        arguments: Mapping[str, str],
        openai_func: OpenaiFunction | None = None,
    ):
        self.func_name = func_name
        self.arg_values = dict()
        self.arg_values.update(arguments)
        # Record the original openai function object,
        # which is used when we want to tell the model that it has
        # previously called this function/tool
        self.openai_func = openai_func or (
            OpenaiFunction(
                arguments=json.dumps(arguments), name=func_name
            ) if OpenaiFunction != Any else None
        )

    def __str__(self):
        return f"Call function `{self.func_name}` with arguments {self.arg_values}."

    def to_dict(self):
        return {"func_name": self.func_name, "arguments": self.arg_values}

    def to_dict_with_result(
        self, call_ok: bool, result: str | None = None, agent_id: str | None = None
    ):
        return {
            "func_name": self.func_name,
            "arguments": self.arg_values,
            "call_ok": call_ok,
            "result": result,
            "agent_id": agent_id,
        }


class MessageThread:
    """
    Represents a thread of conversation with the model.
    Abstracted into a class so that we can dump this to a file at any point.
    """

    def __init__(self, messages=None):
        self.messages: list[dict] = messages or []

    def add(self, role: str, message: str):
        """
        Add a new message to the thread.
        
        Args:
            role: The role of the new message (e.g., "user", "system", "assistant")
            message: The content of the new message
        """
        self.messages.append({"role": role, "content": message})

    def add_system(self, message: str):
        """Add a system message to the thread."""
        self.messages.append({"role": "system", "content": message})

    def add_user(self, message: str):
        """Add a user message to the thread."""
        self.messages.append({"role": "user", "content": message})

    def add_image(self, messages: list[str]):
        """Add image messages to the thread."""
        def encode_image(image_path):
            with open(image_path, "rb") as image_file:
                return base64.b64encode(image_file.read()).decode("utf-8")
            
        for idx, message in enumerate(messages):
            if 'imgur' in message:
                if 'Ow4tDFX' in message:
                    image1_data = encode_image('temp.jpeg')
                else:
                    image1_data = encode_image('temp1.jpeg')
                self.messages.append({
                    "role": "user",
                    "content": [
                        {
                            "type": "text",
                            "text": "<image>:\n"
                        },
                        {
                            "type": "image_url",
                            "image_url": {"url": f"data:image/jpeg;base64,{image1_data}"},
                        }
                    ]
                })
            else:
                message = message.replace('snipboard', 'i.snipboard')
                self.messages.append({
                    "role": "user",
                    "content": [
                        {'type': 'text', 'text': f'<image of {message}>:\n'},
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": message
                            },
                        },
                    ]
                })

    def add_tool(self, message: str, tool_call_id: str):
        """Add a tool response message to the thread."""
        m = {"role": "tool", "content": message, "tool_call_id": tool_call_id}
        self.messages.append(m)

    def add_model(
        self, message: str | None, tools: list[ChatCompletionMessageToolCall] | None = None
    ):
        """Add a model response message to the thread."""
        if tools is None:
            tools = []
        
        # Serialize tools into json first
        json_tools = []
        for tool in tools:
            this_tool_dict = {}
            this_tool_dict["id"] = tool.id
            this_tool_dict["type"] = tool.type
            # Serialize function as well
            func_obj: OpenaiFunction = tool.function
            func_args: str = func_obj.arguments
            func_name: str = func_obj.name
            this_tool_dict["function"] = {"name": func_name, "arguments": func_args}
            json_tools.append(this_tool_dict)

        if json_tools == []:
            # There is no tool calls from the model last time,
            # the best we could do is to return the generated text
            self.messages.append({"role": "assistant", "content": message})
        else:
            self.messages.append(
                {"role": "assistant", "content": None, "tool_calls": json_tools}
            )

    def to_msg(self) -> list[dict]:
        """
        Convert to the format to be consumed by the model.
        
        Returns:
            List[Dict]: The message thread.
        """
        return self.messages

    def __str__(self):
        return pformat(self.messages, width=160, sort_dicts=False)

    def save_to_file(self, file_path: str):
        """
        Save the current state of the message thread to a file.
        
        Args:
            file_path: The path to the file.
        """
        with open(file_path, "w") as f:
            json.dump(self.messages, f, indent=4)

    def get_round_number(self) -> int:
        """
        From the current message history, decide how many rounds have been completed.
        
        Returns:
            Number of completed rounds.
        """
        completed_rounds = 0
        for message in self.messages:
            if message["role"] == "assistant":
                completed_rounds += 1
        return completed_rounds

    @classmethod
    def load_from_file(cls, file_path: str):
        """
        Load the message thread from a file.
        
        Args:
            file_path: The path to the file.
        
        Returns:
            MessageThread: The message thread.
        """
        with open(file_path) as f:
            messages = json.load(f)
        return cls(messages)

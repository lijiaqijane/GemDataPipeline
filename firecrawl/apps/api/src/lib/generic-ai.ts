...
export function getModel(name: string, provider: Provider = defaultProvider) {
  ...
  const modelName = config.MODEL_NAME ? config.MODEL_NAME : name;

  if (provider === "openai") {
    const mode = (config.OPENAI_MODEL_MODE || "chat").toLowerCase();
    if (mode === "chat") {
      return providerList[provider].chat(modelName);
    }
    if (mode === "completion") {
      return providerList[provider].completion(modelName);
    }
  }
